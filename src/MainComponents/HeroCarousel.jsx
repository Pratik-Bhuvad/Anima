...
const HeroCarousel = () => {
  ...
  return (
    <div className='h-[80vh] lg:h-[calc(80vh-4.5rem)] font-mono overflow-hidden relative'>
      ...
      <div className='flex items-center justify-between pl-12 absolute bottom-20 transform -translate-y-1/2 lg:bottom-20 lg:w-[12%]'>
        <Buttons move="left" card={handleSliderLeft} cursor={(currentslide >= totalslides - settings.slidesToShow)?'cursor-not-allowed':'cursor-pointer'}/>
        <Buttons move="right" card={handleSliderRight} cursor={(currentslide === 0)?'cursor-not-allowed':'cursor-pointer'}/>
      </div>
    </div>
  );
}
