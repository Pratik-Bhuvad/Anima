    ...
    return (
        <div className='w-full h-full'>
            <div id="heading" className='flex items-center justify-between h-1/5 md:p-3 md:py-2 '>
                <div id="ratings" className='flex items-center justify-evenly w-[55%] h-full md:w-2/6 *:flex *:items-center *:justify-center *:w-1/3 *:h-full *:gap-x-2'>
                    <span>{anime[0].saved}<img src="/bookmark.png" alt="" className="w-2/5 h-2/3 md:w-2/4 md:h-full object-fit" /></span>
            ...
            <div id="cards" className='grid grid-cols-3 md:grid-cols-4 gap-x-3 overflow-hidden w-full h-4/5'>
                {anime.slice(0, cards).map((anime, index) => (
                    <img src={anime.card_url} alt="" className='h-[95%] rounded-xl' />
                ))
                }
            </div>
        </div>
