...
const Showinfo = ({ anime }) => {
    ...
    return (
        <div className='h-screen flex flex-col items-center gap-x-5 gap-y-3 pr-5 pl-5 py-5 *:h-3/5 lg:h-[90vh] lg:py-0 lg:*:h-[70%] lg:flex-row-reverse lg:gap-y-0' style={{ background: `linear-gradient(rgba(0,0,0,0.6),rgba(0,0,0,0.6)), url(${anime.backdrop_url}) center/cover no-repeat` }}>
            <div className="h-full relative group *:rounded-2xl">
                <img src={anime.card_url} alt="" className='h-full object-contain rounded-3xl' />
                <div className="w-full h-full flex items-center justify-center absolute opacity-0 top-0 bg-[#18181871] group-hover:opacity-100 transition-opacity duration-300 md:!flex">
                ...
                {shared && (
                    <div id='shared' className='absolute top-3 right-5 !rounded-lg bg-[#000000c0] p-1 px-4 tracking-normal text-[#FFA629]'>
                        shared <i className="fa-solid fa-crown ml-1"></i>
                    </div>
                )}
                <button className="absolute left-1/4 bottom-2 bg-[#FFa629] text-xs !rounded-md w-3/6 h-[10%] flex items-center justify-evenly lg:text-2xl lg:-bottom-7 text-black px-2">Play Online <i className="bi bi-play text-2xl lg:text-4xl"></i></button>
            </div>
            <div id="info" className='h-full flex w-full flex-col gap-y-1 justify-between lg:items-end lg:w-3/5'>
            ...
            <div className='w-full lg:!h-1/2 flex justify-between lg:flex-col lg:w-1/5'>
                <div className="offical-rating w-full flex flex-col justify-evenly *:w-[32%] *:flex *:items-center *:justify-start lg:text-3xl lg:flex-row font-bold *:*:mr-3">
                ...
                <div className="user-rating w-full px-4 flex flex-col items-center justify-center tracking-wide text-xs lg:text-lg font-semibold lg:w-3/4">
                    <StarRating />
