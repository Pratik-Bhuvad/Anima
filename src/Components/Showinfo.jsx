    ...
    return (
        <div className='h-screen flex flex-col items-center gap-x-5 gap-y-3 pr-5 pl-5 py-5 *:h-3/5 md:*:h-4/5  md:grid md:grid-cols-2 lg:flex lg:h-[90vh] lg:py-0 lg:*:h-[65%] lg:flex-row-reverse lg:gap-y-0' style={{ background: `linear-gradient(rgba(0,0,0,0.6),rgba(0,0,0,0.6)), url(${anime.backdrop_url}) center/cover no-repeat` }}>
            <div className="h-full relative group *:rounded-2xl">
                <img src={anime.card_url} alt="" loading='lazy' className=' h-full object-contain rounded-2xl' />
            ...
            <div id="info" className='h-full flex w-full flex-col gap-y-1 justify-between md:justify-center md:gap-y-5 lg:justify-evenly lg:items-end lg:w-2/5 xl:w-3/5'>
                ...
                <div id="status" className='w-4/5 xl:w-[37%] hidden justify-evenly items-center xl:text-xl py-3 rounded-2xl bg-[#2a2b2e] lg:flex'>Status: {anime.currentStatus} <i className={`fa-solid fa-xl ${ anime.seasonStatus === 'completed' ? 'fa-circle-check' : anime.seasonStatus === 'playing' ? 'fa-circle-play' : anime.seasonStatus === 'hiatus' ? 'fa-pause' : anime.seasonStatus === 'announced' ? 'fa-bullhorn' :'fa-circle-question'}`}></i></div>
            </div>
            <div className='w-full lg:!h-1/2 flex justify-between md:col-span-2 md:gap-x-10  lg:flex-col lg:w-1/4 xl:w-1/5 lg:gap-0'>
                <div className="offical-rating w-full flex flex-col justify-evenly  *:w-[32%] *:flex *:items-center *:justify-start  md:items-center xl:text-3xl lg:flex-row font-bold *:*:mr-3">
                    <span><img src="/mal.png" alt="" loading='lazy' className='rounded-full w-7 h-7 xl:w-12 xl:h-12' />{anime.rating}</span>
                    <span><img src="/imdb.png" alt="" loading='lazy' className=' w-10 h-7 xl:w-12 xl:h-8' />{anime.rating}</span>
                    <span className='relative -left-1 xl:left-0'> <div className='group h-full'><img src="/bookmark.png" alt="" loading='lazy' className='w-7 h-7 xl:w-12 xl:h-12 cursor-pointer group max-w-max' /> <span className='absolute invisible xl:-top-9 xl:-left-8 xl:w-full rounded-md xl:p-2 bg-gray-600 text-center text-xs font-normal duration-150 xl:group-hover:visible'>Add to Watch list</span></div> {anime.saved}</span>
                </div>
